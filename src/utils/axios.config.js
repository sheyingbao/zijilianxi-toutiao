...
axios.interceptors.request.use(function (config) {
  // 在发起请求请做一些业务处理
  // 进行配置的注入
  const userInfo = window.localStorage.getItem('user-info') // 获取用户存储信息 存储信息里有token
  const token = userInfo ? JSON.parse(userInfo).token : null // 获取token
...
axios.interceptors.response.use(function (response) {
  ...
  // 对响应错误做处理
  const code = error.response ? error.response.status : '' // 获取错误的状态码
  let message = ''
  switch (code) {
    case 400:
      message = '请求参数错误'
